  ...
  try {
    ...
    await provider.request({
      ...
      params: [{
        ...
        rpcUrls: [RPC],
        blockExplorers: [{ name: 'GenLayer Explorer', url: 'https://explorer-studio.genlayer.com' }],
      }],
    });
  }
}

function zeroFeeProvider(provider: Eip1193Provider): Eip1193Provider {
  return {
...
export function OnchainProvider({ children, initialSnapshot = null }: { children: ReactNode; initialSnapshot?: OnchainSnapshot | null }) {
  ...
  const execute = useCallback(async (functionName: string, args: unknown[]) => {
    ...
    const client = createClient({
      ...
      provider: zeroFeeProvider(provider) as never,
    });
    ...
    setLastTx(hash);
    await client.waitForTransactionReceipt({ hash, status: 'CLOSED' as never, retries: 200 });
  ...
  return (
    <OnchainContext.Provider value={contextValue}>
      ...
      {dockOpen && (
        <div className="chain-dock-scrim chain-dock-scrim--recall-loop" role="presentation" onMouseDown={event => {
          if (event.currentTarget === event.target) setDockOpen(false);
        }}>
          <section className="chain-dock chain-dock--recall-loop" data-panel="recall-loop" role="dialog" aria-modal="true" aria-label="Onchain contract controls">
            <header>
              <div><span>RECALL COMMAND</span><b>{snapshot.contract}</b></div>
            ...
            {mode === 'create' && (
              <div className="chain-fields">
                <label>Title<input value={title} onChange={event => setTitle(event.target.value)} placeholder="Affected product" /></label>
                <label>Claim<textarea value={claim} onChange={event => setClaim(event.target.value)} placeholder="Official recall guidance supports notifying affected distribution destinations and documenting containment actions before closure." /></label>
                <label>Public source URL<input value={sourceUrl} onChange={event => setSourceUrl(event.target.value)} placeholder="https://..." /></label>
                <button type="button" disabled={Boolean(busy) || !title.trim() || !claim.trim() || !/^https?:\/\//i.test(sourceUrl)} onClick={() => void run('open_recall', [title.trim(), "Recall authority", claim.trim(), sourceUrl.trim()])}>
                  {busy === 'open_recall' ? 'Waiting for finality' : 'Open recall onchain'}
                </button>
              </div>
            )}
            ...
            {mode === 'evidence' && selected && (
              <div className="chain-fields">
                ...
                <button type="button" disabled={Boolean(busy) || !operator || !title.trim() || !/^https?:\/\//i.test(sourceUrl)} onClick={() => void run('add_evidence', [selected.id, sourceUrl.trim(), title.trim(), note.trim()])}>
                  {busy === 'add_evidence' ? 'Waiting for finality' : operator ? 'Add evidence onchain' : 'Recall operator required'}
                </button>
              </div>
            )}
            ...
            {mode === 'review' && selected && (
              <div className="chain-fields">
                ...
                <button type="button" disabled={Boolean(busy) || !operator || selected.status !== 'ACTIVE' || selected.evidenceCount === 0} onClick={() => void run('review_with_genlayer', [selected.id])}>
                  {busy === 'review_with_genlayer' ? 'Validators are reviewing' : !operator ? 'Recall operator required' : selected.status !== 'ACTIVE' ? `Record is ${selected.status}` : 'Run validator review'}
                </button>
              </div>
            )}
            ...
            {mode === 'challenge' && selected && (
              <div className="chain-fields">
                ...
                <button type="button" disabled={Boolean(busy) || selected.status !== 'OBJECTION_WINDOW' || !claim.trim() || !/^https?:\/\//i.test(sourceUrl)} onClick={() => void run('submit_challenge', [selected.id, claim.trim(), sourceUrl.trim()])}>
                  {busy === 'submit_challenge' ? 'Waiting for finality' : selected.status !== 'OBJECTION_WINDOW' ? `Record is ${selected.status}` : 'Submit challenge onchain'}
                </button>
            ...
            {mode === 'appeal' && selected && (
              <div className="chain-fields">
                ...
                <label>Public evidence URL<input value={sourceUrl} onChange={event => setSourceUrl(event.target.value)} placeholder="https://..." /></label>
                <button type="button" disabled={Boolean(busy) || !['ASSESSED', 'OBJECTION_WINDOW'].includes(selected.status) || !note.trim() || !/^https?:\/\//i.test(sourceUrl)} onClick={() => void run('submit_appeal', [selected.id, note.trim(), sourceUrl.trim()])}>
                  {busy === 'submit_appeal' ? 'Waiting for finality' : !['ASSESSED', 'OBJECTION_WINDOW'].includes(selected.status) ? `Record is ${selected.status}` : 'Submit appeal onchain'}
                </button>
            ...
            {mode === 'lifecycle' && selected && (
              <div className="chain-fields chain-lifecycle">
                <button type="button" disabled={Boolean(busy) || !operator || selected.status !== 'ASSESSED'} onClick={() => void run('open_challenge_window', [selected.id])}>Open challenge window</button>
                <button type="button" disabled={Boolean(busy) || !operator || !['ASSESSED', 'OBJECTION_WINDOW'].includes(selected.status)} onClick={() => void run('finalize_case', [selected.id])}>Finalize eligible record</button>
                <button type="button" disabled={Boolean(busy) || !operator || selected.status !== 'CLOSED'} onClick={() => void run('archive_case', [selected.id])}>Archive finalized record</button>
